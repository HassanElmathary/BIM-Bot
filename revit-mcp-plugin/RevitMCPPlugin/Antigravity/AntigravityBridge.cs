...
{
    /// <summary>
    /// Direct TCP bridge to the Revit MCP service on localhost:8080.
    ...
    public class AntigravityBridge
    {
        ...
        private static string CallRevitTcp(string method, string paramsJson)
        {
            ...
            catch (AggregateException ex)
            {
                ...
                return $"{{\"error\":{{\"message\":\"Cannot connect to Revit MCP on port 8080. Start the MCP service first. ({inner})\"}}}}";
            }
            catch (Exception ex)
            {
