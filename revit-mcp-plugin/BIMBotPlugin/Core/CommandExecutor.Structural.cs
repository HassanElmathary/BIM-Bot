    ...
    public static partial class CommandExecutor
    {
        private static JToken CreateStructuralBeam(Document doc, JObject parameters)
        {
            ...
            using (var tx = new Transaction(doc, "Create Beam"))
            {
                ...
                tx.Commit();
                return new JObject { ["message"] = $"🏗️ Created beam (ID: {beam.Id.Val()})", ["elementId"] = beam.Id.Val() };
            }
        }
        ...
        private static JToken CreateStructuralColumn(Document doc, JObject parameters)
        {
            ...
            using (var tx = new Transaction(doc, "Create Column"))
            {
                ...
                tx.Commit();
                return new JObject { ["message"] = $"🏗️ Created column (ID: {col.Id.Val()})", ["elementId"] = col.Id.Val() };
            }
        }
        ...
        private static JToken CreateWallFoundation(Document doc, JObject parameters)
        {
            ...
            var wall = doc.GetElement(wallId.ToElementId()) as Wall;
            ...
            return new JObject { ["message"] = $"🏗️ Wall foundation: {foundTypes.Count} types available for wall {wallId}", ["types"] = JArray.FromObject(foundTypes.Select(t => new { t.Name, id = t.Id.Val() })), ["hint"] = "Use execute_code: doc.Create.NewFamilyInstance(curve, foundationType, wall, level, StructuralType.Footing)" };
        }

        private static JToken CreateRebar(Document doc, JObject parameters)
        {
            var hostId = parameters["hostId"]?.Value<int>() ?? 0;
            var host = doc.GetElement(hostId.ToElementId());
            ...
            return new JObject { ["message"] = $"🏗️ {barTypes.Count} rebar types available for '{host.Name}'", ["barTypes"] = JArray.FromObject(barTypes.Select(b => new { b.Name, id = b.Id.Val() })), ["hint"] = "Use execute_code: Rebar.CreateFromCurves(doc, rebarStyle, barType, hookType, hookType, host, normal, curves, hookOrient, hookOrient, useExistingShape, createNewShape)" };
        }
        ...
        private static JToken GetStructuralElements(Document doc, JObject parameters)
        {
            ...
            foreach (var e in elements.Take(100))
                items.Add(new JObject { ["id"] = e.Id.Val(), ["name"] = e.Name, ["family"] = (e as FamilyInstance)?.Symbol?.Family?.Name });
        ...
        private static JToken AnalyticalModelInfo(Document doc, JObject parameters)
        {
            ...
            if (!string.IsNullOrEmpty(idsStr))
                ...
                {
                    var elem = doc.GetElement((int.Parse(idStr.Trim())).ToElementId());
                    if (elem == null) continue;
                    var item = new JObject { ["id"] = elem.Id.Val(), ["name"] = elem.Name, ["category"] = elem.Category?.Name };
