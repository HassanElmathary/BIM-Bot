...
namespace BIMBotPlugin.Core
{
    ...
    public static partial class CommandExecutor
    {
        private static JToken CreateStairs(Document doc, JObject parameters)
        {
            ...
            return new JObject { ["message"] = $"🪜 Stairs: {baseLevel.Name} → {topLevel.Name} (height: {Math.Round(topLevel.Elevation - baseLevel.Elevation, 2)}ft)", ["types"] = JArray.FromObject(stairTypes.Select(t => new { t.Name, id = t.Id.Val() })), ["hint"] = "Use execute_code: StairsEditScope + StairsRun.CreateStraightRun()" };
        }
        ...
        private static JToken CreateRailing(Document doc, JObject parameters)
        {
            ...
            return new JObject { ["message"] = $"🛡️ {railTypes.Count} railing types available", ["types"] = JArray.FromObject(railTypes.Select(t => new { t.Name, id = t.Id.Val() })), ["hint"] = "Use execute_code: Railing.Create(doc, curveLoop, railingTypeId, levelId)" };
        }
        ...
        private static JToken CreateCurtainWall(Document doc, JObject parameters)
        {
            ...
            using (var tx = new Transaction(doc, "Create Curtain Wall"))
            {
                ...
                tx.Commit();
                return new JObject { ["message"] = $"🏗️ Created curtain wall (ID: {wall.Id.Val()})", ["elementId"] = wall.Id.Val() };
            }
        }
        ...
        private static JToken CreateShaftOpening(Document doc, JObject parameters)
        {
            ...
            using (var tx = new Transaction(doc, "Create Shaft"))
            {
                ...
                tx.Commit();
                return new JObject { ["message"] = $"🕳️ Created shaft opening (ID: {opening.Id.Val()})", ["elementId"] = opening.Id.Val() };
            }
        }
        ...
        private static JToken GetStairsInfo(Document doc, JObject parameters)
        {
            ...
            var stairs = stairsIdParam.HasValue ? new[] { doc.GetElement((stairsIdParam.Value).ToElementId()) }.Where(e => e != null).ToList() : new FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_Stairs).WhereElementIsNotElementType().ToList();
            var items = new JArray();
            foreach (var s in stairs)
            {
                var item = new JObject { ["id"] = s.Id.Val(), ["name"] = s.Name };
        ...
        {
            var wall = doc.GetElement((parameters["wallId"]?.Value<int>() ?? 0).ToElementId()) as Wall;
            ...
            var panels = new JArray(); foreach (var pId in cg.GetPanelIds()) { var p = doc.GetElement(pId); panels.Add(new JObject { ["id"] = p.Id.Val(), ["name"] = p?.Name }); }
            var mullions = new JArray(); foreach (var mId in cg.GetMullionIds()) { var m = doc.GetElement(mId); mullions.Add(new JObject { ["id"] = m.Id.Val(), ["name"] = m?.Name }); }
        ...
        private static JToken CreateOpeningInWall(Document doc, JObject parameters)
        {
            ...
            using (var tx = new Transaction(doc, "Create Opening"))
            {
                ...
                tx.Commit();
                return new JObject { ["message"] = $"🕳️ Created wall opening (ID: {opening.Id.Val()})", ["elementId"] = opening.Id.Val() };
            }
        }
    }
}
