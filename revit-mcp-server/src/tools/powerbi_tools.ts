...
export function registerPowerBITools(server: McpServer) {
    ...
    server.tool(
        ...
        "The SQLite file can be loaded into Power BI Desktop via ODBC/SQLite connector and used with the BIMBot Viewer custom visual.",
        {
            ...
            dbPath: z.string().optional().describe(
                "Full path for the SQLite .db file. Default: data/BIMBot_PowerBI.db (next to the .rvt file)"
            ),
        ...
        async (args) => {
            try {
                ...
                const text = [
                    ...
                    "3. Import the BIMBot Viewer custom visual (.pbiviz)",
